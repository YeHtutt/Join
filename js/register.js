async function signUpUser() {
  ...
  if (splitName.length !== 2) {
    warnForInputFirstAndLastName();
  } else {
    savedTheSignedUpUser(name, email, password, userId, userInitials, userColor, splitName);
  }
}
...
function pushNewUser(name, email, password, userId, userInitials, userColor) {
  let newUser = {
    ...
    userTasks: [],
    userCategory: [{ 'category': 'Sales', 'color': '#FC71FF', }, { 'category': 'Backoffice', 'color': '#1FD7C1', }
    ]
  };
  userAccounts.push(newUser);
}

function warnForInputFirstAndLastName() {
...
async function savedTheSignedUpUser(name, email, password, userId, userInitials, userColor, splitName) {
